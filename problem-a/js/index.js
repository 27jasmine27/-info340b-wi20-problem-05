...
let h1 = document.querySelector("h1");
  // h1.innerHTML = "<span class='unread'/>" : "");
...
*/
function renderPaletteRow(colorArray, parentDom) {
  let heightAndWidth = parentDom.getBoundingClientRect();
  let colorBox = document.createElement('div');
  for(let color of colorArray) {
    let newDiv = createColorBox(color, (heightAndWidth.width/ colorArray.size));
    colorBox.appendChild(newDiv);
  }
  parentDom.appendChild(colorBox);
}
...
function renderPaletteTable() {
  ...
  for (let paletteRow in COLORS_9) {
